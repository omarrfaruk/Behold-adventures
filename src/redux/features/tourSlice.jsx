...
export const updateTour = createAsyncThunk('tour/updateTour', async ({ id, toast, updatedTourData, navigate }, { rejectWithValue }) => {
    try {
...
export const toursByTag = createAsyncThunk(
    "tour/toursByTag",
    async (tag, { rejectWithValue }) => {
        try {
            const response = await api.getTourByTag(tag);
            return response.data;
        } catch (err) {
            return rejectWithValue(err.response.data);
        }
...
const tourSlice = createSlice({
    ...
    extraReducers: {
        ...
        [toursByTag.pending]: (state, action) => {
            state.loading = true;
        },
        [toursByTag.fulfilled]: (state, action) => {
            state.loading = false;
            state.tagTours = action.payload;
        },
        [toursByTag.rejected]: (state, action) => {
            state.loading = false;
            state.error = action.payload.message;
        },
